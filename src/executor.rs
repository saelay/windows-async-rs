...
const WM_USER_ASYNCACT_COMPLETED:u32 = WM_USER + 2;

static mut REGISTER_ATOM: Option<u16> = None;
static mut DUMMY_WINDOW: Option<DummyWindow> = None;


pub(crate) fn post_message_asyncop_completed() {
    unsafe {
        if let Some(dummy_window) = DUMMY_WINDOW.as_ref() {
...
pub(crate) fn post_message_asyncact_completed() {
    unsafe {
        if let Some(dummy_window) = DUMMY_WINDOW.as_ref() {
            log::trace!("PostMessage(WM_USER_ASYNCACT_COMPLETED) to {:?}", dummy_window);
            PostMessageW(dummy_window.hwnd(), WM_USER_ASYNCACT_COMPLETED, WPARAM(0), LPARAM(0));
        }
    }
}
...
extern "system" fn wndproc(hwnd: HWND, msg: u32, wparam: WPARAM, lparam: LPARAM) -> LRESULT {
    match msg {
        WM_USER_ASYNCOP_COMPLETED => {
            log::trace!("AsyncOperation Completed ({:?})", hwnd);
        }
        WM_USER_ASYNCACT_COMPLETED => {
            log::trace!("AsyncAction Completed ({:?})", hwnd);
        }
        _ => {}
    }
...
pub fn block_on<T>(mut f: impl Future<Output=T>) -> T {
    ...
    unsafe {
        if DUMMY_WINDOW.is_none() {
            DUMMY_WINDOW = Some(create_dummy_window());
        }
    }
